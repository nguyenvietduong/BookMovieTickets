...
class BaseRepository implements BaseRepositoryInterface
{
    ...
    public function pagination(
        ...
        // Lấy cột tìm kiếm từ condition, nếu không có thì sử dụng giá trị mặc định ['name']
        $searchColumns = $condition['searchColumns'] ?? ['name'];

        return $query
            ->keyword($condition['keyword']         ?? null, $searchColumns)        //Lọc kết quả dựa trên từ khóa nếu có
    ...
    {
        // Kiểm tra xem phương thức liên quan có tồn tại trên đối tượng model không
        if (method_exists($model, $relation)) {
            // Gọi phương thức liên quan và thực hiện attach
            $model->{$relation}()->attach($data);
        } else {
            throw new \Exception("Relation {$relation} does not exist on the given model.");
        }
    }
    ...
    public function sync($model, $relation, $data)
    {
        // Kiểm tra xem phương thức liên quan có tồn tại trên đối tượng model không
        if (method_exists($model, $relation)) {
            // Gọi phương thức liên quan và thực hiện sync
    ...
    public function getFields(array $columns = ['*'])
    {
    ...
    {
        return $this->model->withCount($relations)->get();
    }
